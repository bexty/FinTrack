...
INITIALPERIOD = 'Month' #режим, в котором стартует приложение по умолчанию, D = Date, M = Month, Y = Year, A = All time
INITIALVIEWMODE = 'Grouped by category' #'Sort by transaction date', 'Sort by transaction ID'
CURRENCYSYMBOL =  '₽'
INITIALDATEFORMAT = 'Europe'
DEFAULTSETTINGSID = 0
USERSETTINGSID = 1
...
class GUI():
    ...
    def periodButtonAction(self, sender):
        date = [{'type':'date', 'title':'Date:', 'key':'date', 'format':LOCALDATEFORMAT[self.controller.localDateFormat]['day']}]

        period = [{'type':'check', 'title':'Day', 'group':'period', 'value':True if self.controller.period == 'Day' else False},
                {'type':'check', 'title':'Month', 'group':'period', 'value':True if self.controller.period == 'Month' else False},
                {'type':'check', 'title':'Year', 'group':'period', 'value':True if self.controller.period == 'Year' else False},
                {'type':'check', 'title':'All time', 'group':'period', 'value':True if self.controller.period == 'All time' else False},
                {'type':'check', 'title':'Custom period', 'group':'period', 'value':True if self.controller.period == 'Custom period' else False},
                {'type':'date', 'title':'From:', 'key':'fromDate', 'format':LOCALDATEFORMAT[self.controller.localDateFormat]['day']},
                {'type':'date', 'title':'To:', 'key':'toDate', 'format':LOCALDATEFORMAT[self.controller.localDateFormat]['day']}]

        viewMode = [{'type':'check', 'title':'Grouped by category', 'group':'viewMode', 'value':True if self.controller.viewMode == 'Grouped by category' else False},
                    {'type':'check', 'title':'Sort by transaction date', 'group':'viewMode', 'value':True if self.controller.viewMode == 'Sort by transaction date' else False},
                    {'type':'check', 'title':'Sort by transaction ID', 'group':'viewMode', 'value':True if self.controller.viewMode == 'Sort by transaction ID' else False}]

        items = [('Date:', date),
                ('View period:', period),
                ('View mode:', viewMode)]

        a = dialogs.form_dialog(title='View options', sections=items)
        if a:
            self.controller.periodButtonAction(a)

    @ui.in_background
    def settingsButtonAction(self, sender):
        initialPeriod = [{'type':'check', 'title':'Day', 'group':'initialPeriod', 'value':True if self.controller.initialPeriod == 'Day' else False},
                        {'type':'check', 'title':'Month', 'group':'initialPeriod', 'value':True if self.controller.initialPeriod == 'Month' else False},
                        {'type':'check', 'title':'Year', 'group':'initialPeriod', 'value':True if self.controller.initialPeriod == 'Year' else False},
                        {'type':'check', 'title':'All time', 'group':'initialPeriod', 'value':True if self.controller.initialPeriod == 'All time' else False}]
        
        dateFormats = [{'type':'check', 'title':'Europe', 'group':'localDateFormat', 'value':True if self.controller.localDateFormat == 'Europe' else False},
                        {'type':'check', 'title':'Japan', 'group':'localDateFormat', 'value':True if self.controller.localDateFormat == 'Japan' else False},
                        {'type':'check', 'title':'USA', 'group':'localDateFormat', 'value':True if self.controller.localDateFormat == 'USA' else False}]

        currencySymbol = [{'type':'text', 'title':'Currency symbol:\t', 'key':'currencySymbol', 'value':self.controller.currencySymbol}]
        
        categories = []
        for i in self.controller.categories:
        ...
        items = [('Start mode:', initialPeriod),
    ...
    def addButtonAction(self, sender, transactionData=None):
        if transactionData: #если редактируем существующую транзакцию
            ...
            mainData = [{'type':'number', 'title':'Price:\t', 'key':'price', 'value':str(transactionData['price'])},
                ...
                {'type':'date', 'title':'Date:\t', 'key':'date', 'value':datetime.datetime.strptime(transactionData['date'], '%Y-%m-%d %H:%M:%S'), 'format':LOCALDATEFORMAT[self.controller.localDateFormat]['day']},
            ...
            mainData = [{'type':'number', 'title':'Price:\t', 'key':'price'},
                ...
                {'type':'date', 'title':'Date:\t', 'key':'date', 'format':LOCALDATEFORMAT[self.controller.localDateFormat]['day']},
...
class Model(): #sqlite3 data access layer
    ...
    def createDb(self):
        ...
        test = cur.execute('''SELECT * FROM settings WHERE id=?''', (DEFAULTSETTINGSID, )).fetchone() #проверяем на наличие записей в таблице, если записей нет, и таблица была только что создана - добавляем
        if not test:
    ...
    def getTransactionsFromAllTime(self, category):
        connection = sqlite3.connect(DB)
        connection.row_factory = sqlite3.Row
        db = connection.cursor()
        transactions = db.execute('''SELECT id as id,
    ...
    def getSettings(self, mode='user'):
        ...
        connection.row_factory = sqlite3.Row
        db = connection.cursor()
        settings = db.execute('''SELECT datetime("now", "localtime") as now,
            categories as categories,
            initialPeriod as initialPeriod,
            initialViewMode as initialViewMode,
    ...
    def updateSettings(self):
        ...
        db.execute('''UPDATE settings SET categories=?,
            ...
            initialViewMode=?,
            currencySymbol=?,
            localDateFormat=? WHERE id=?''', ('\t'.join(self.controller.categories),
                self.controller.initialPeriod,
                self.controller.initialViewMode,
                self.controller.currencySymbol,
                self.controller.localDateFormat,
                USERSETTINGSID))
        connection.commit()
        connection.close()
...
class Controller():
    def __init__(self):
        ...
        self.categories = None #list of categories
        self.period = None
        self.initialPeriod = None
        self.viewMode = None
        self.initialViewMode = None
        self.currencySymbol = None
        self.localDateFormat = None

        self.getSettings() #read and update user settings from db
    ...
    def periodButtonAction(self, result):
        ...
        self.fromDate = result['fromDate']
        self.toDate = result['toDate']
        self.viewMode = result['viewMode']
        self.gui.categoriesTableUpdate(self.getNameForMainView(), self.getItemsForCategoriesTable())     
    ...
    def settingsButtonAction(self, settings):
        if settings['reset'] == True:
            ...
            self.initialPeriod = settings['initialPeriod']
            self.localDateFormat = settings['localDateFormat']
            self.currencySymbol = settings['currencySymbol']
            if settings.get('newCategories', None):
                self.categories = settings['newCategories']
    ...
    def addButtonAction(self, transaction, transactionId=None):
        if transaction['newCategory'] != '': #у нас есть новая категория
            ...
            self.categories.append(transaction['newCategory'])
            self.model.updateSettings()
    ...
    def categoriesTableRowSelectedAction(self, category):
        ...
        for row in transactions:
            ...
            string['id'] = row['id']
            date = datetime.datetime.strptime(row['date'], DEFAULTFORMATDAY).strftime(LOCALDATEFORMAT[self.localDateFormat]['day']) #меняем дату под локаль
    ...
    def floatToPrice(self, fPrice, mode='rough'):
        if fPrice == None:
            ...
            return (format(fPrice, ',.0f').replace(',', '  ')+ ' '+self.currencySymbol)
        elif mode == 'precisely':
            return (format(fPrice, ',.2f').replace(',', '  ')+' '+self.currencySymbol)
        else:
    ...
    def getNameForMainView(self):
        if self.period == 'Day':
            ...
            return (self.date.strftime(LOCALDATEFORMAT[self.localDateFormat]['day'])+' ('+self.floatToPrice(total)+')')
        elif self.period == 'Month':
            period = ((self.date.strftime('%Y-%m')+'-01'), (self.date.strftime('%Y-%m')+'-31'))
            total = self.model.getTotalPriceFromPeriod(period)
            return (self.date.strftime(LOCALDATEFORMAT[self.localDateFormat]['month'])+' ('+self.floatToPrice(total)+')')
        elif self.period == 'Year':
            period = ((self.date.strftime('%Y')+'-01-01'), (self.date.strftime('%Y')+'-12-31'))
            total = self.model.getTotalPriceFromPeriod(period)
            return (self.date.strftime(LOCALDATEFORMAT[self.localDateFormat]['year'])+' ('+self.floatToPrice(total)+')')
        elif self.period == 'Custom period':
            period = (self.fromDate.strftime('%Y-%m-%d'), self.toDate.strftime('%Y-%m-%d'))
            total = self.model.getTotalPriceFromPeriod(period)
            return (self.fromDate.strftime(LOCALDATEFORMAT[self.localDateFormat]['short'])+' - '+self.toDate.strftime(LOCALDATEFORMAT[self.localDateFormat]['short'])+' ('+self.floatToPrice(total)+')')
        elif self.period == 'All time':
    ...
    def getSettings(self, mode='user'):
        ...
        self.date = datetime.datetime.strptime(a['now'], '%Y-%m-%d %H:%M:%S')
        self.categories = a['categories'].split('\t')
        self.period = a['initialPeriod']
        self.initialPeriod = a['initialPeriod']
        self.viewMode = a['initialViewMode']
        self.initialViewMode = a['initialViewMode']
        self.currencySymbol = a['currencySymbol']
        self.localDateFormat = a['localDateFormat']
